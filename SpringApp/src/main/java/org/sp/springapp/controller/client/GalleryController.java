...
public class GalleryController {
	...
	public String regist(Gallery gallery, HttpServletRequest request) {
		...
		List<String> imgList = new ArrayList<String>(); // 새롭게 생성한 파일명이 누적될 곳
	...
	public ModelAndView getContent(int gallery_idx) {
		...
		// 4단계 : 가져온 레코드 저장(jsp로 가져갈려고..)
		ModelAndView mav = new ModelAndView();
		mav.addObject("gallery", gallery); // request.setAttribute
	...
	public String del(int gallery_idx, String[] filename, HttpServletRequest request) {
		...
		return "redirect:gallery/list"; // WEB-INF/views/gallery/list.jsp
	}

	// 어떠한 예외가 발생했을때, 어떤 처리를 할지 아래의 메서드에서 로직작성..
